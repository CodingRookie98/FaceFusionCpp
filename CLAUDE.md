...
2. **阻断机制**：在执行任何代码修改前，**必须**先阅读并执行工作流程文档。
3. **语言锁定**：无论用户使用何种语言提问，**必须** 使用中文进行思考和回答（代码注释除外）。

## 📋 工作流程 (MANDATORY)

**在执行任何 [代码修改] 或 [重构任务] 之前，你必须执行以下动作：**

1. 🛑 **HALT**：停止所有代码编写意图。
2. 📖 **READ**：使用 `read` 工具读取 `docs/dev_docs/workflow.md`。
3. ✅ **CHECK**：严格按照 `workflow.md` 中的 Checklists 逐项执行（切分支 -> 写计划 -> 获批准 -> 编码）。

> **违规警告**：未读取 `workflow.md` 而直接修改代码将被视为严重违规。

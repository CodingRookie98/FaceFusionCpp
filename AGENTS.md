...
4. **TDD 强制**：任何代码修改必须遵循 TDD 流程，无测试不写代码。
...
1. 🛑 **HALT**：停止所有代码编写意图。
2. 📖 **READ**：使用 `read` 工具读取 @docs/dev/process/workflow.md。
3. ✅ **CHECK**：严格按照 @docs/dev/process/workflow.md 中的 Checklists 逐项执行（切分支 -> 写计划 -> 获批准 -> 编码）。

> **违规警告**：在触发工作流程的情况下，未读取 @docs/dev/process/workflow.md 而直接修改代码将被视为严重违规。
...
- **文档管理** (`/docs/dev/`)：
  ...
  - 任务：`plan/{name}/task/C++_task_{title}.md` (使用 `@docs/dev/templates/C++_task_template.md`)
  - 评估：`evaluation/C++_evaluation_{title}.md` (使用 `@docs/dev/templates/C++_evaluation_template.md`)

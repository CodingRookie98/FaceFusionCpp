...
- **提交要求**：
  ...
  - 文件重命名/移动必须使用 `git mv`。
  - 提交前必须通过 `python build.py --action test --test-label unit`。**纯文档变更除外**（仅修改 `docs/` 目录、`.md` 文件、注释等非代码文件时，无需运行测试即可提交）。
  - **测试职责划分**：智能体/AI 负责运行全部测试（单元测试、集成测试、E2E 测试）。各类测试的执行时机见 `workflow.md`。
...
- **无测试即提交**：在未运行单元测试并通过前进行 git commit。**纯文档变更除外**（仅修改 `docs/`、`.md`、注释等非代码文件时可直接提交）。集成测试和 E2E 测试必须在合并前全部通过。

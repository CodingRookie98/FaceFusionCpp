...
TEST(FrameEnhancerFactoryTest, CreateRealEsrGan) {
    ...
    try {
        auto enhancer =
            FrameEnhancerFactory::create(FrameEnhancerType::RealEsrGan, "real_esrgan_x4", opts);
        ...
        EXPECT_EQ(err.find("model is not supported"), std::string::npos)
...
TEST(FrameEnhancerFactoryTest, CreateRealHatGan) {
    ...
    try {
        auto enhancer =
            FrameEnhancerFactory::create(FrameEnhancerType::RealHatGan, "real_hatgan_x4", opts);
        if (enhancer) EXPECT_NE(enhancer, nullptr);
    } catch (const std::exception& e) {
        std::string err = e.what();
        EXPECT_EQ(err.find("model is not supported"), std::string::npos)

...
protected:
    ...
    void SetUp() override {
        ...
        EXPECT_CALL(*mock_session, get_input_node_dims()).WillRepeatedly(Return(std::vector<std::vector<int64_t>>{{1, 3, -1, -1}}));
    }
...
TEST_F(FrameEnhancerImplTest, EnhanceFrameSimple1x) {
    ...
    EXPECT_CALL(*mock_session, run(_)).WillRepeatedly([&](const std::vector<Ort::Value>&) {
        ...
        outs.push_back(Ort::Value::CreateTensor<float>(
             mem, output_data.data(), output_size, output_shape.data(), output_shape.size()));
        return outs;
    });
...
TEST_F(FrameEnhancerImplTest, EnhanceFrameWithScaling2x) {
    ...
    EXPECT_CALL(*mock_session, run(_)).WillRepeatedly([&](const std::vector<Ort::Value>&) {
        ...
        std::vector<Ort::Value> outs;
        outs.push_back(Ort::Value::CreateTensor<float>(
             mem, static_output_data.data(), output_size, output_shape.data(), output_shape.size()));
        return outs;
    });
...
TEST_F(FrameEnhancerImplTest, EnhanceFrameWithTiling) {
    ...
    EXPECT_CALL(*mock_session, run(_)).WillRepeatedly([&](const std::vector<Ort::Value>& inputs) {
        ...
        std::vector<Ort::Value> outs;
        outs.push_back(Ort::Value::CreateTensor<float>(
             mem, data, out_size, out_shape.data(), out_shape.size()));
        return outs;
    });

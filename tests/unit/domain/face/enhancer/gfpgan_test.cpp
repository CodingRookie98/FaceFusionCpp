...
class GfpGanTest : public ::testing::Test {
protected:
    void SetUp() override {
...
TEST_F(GfpGanTest, LoadModelAndEnhanceFace) {
    std::string model_path = "gfpgan.onnx";
    InferenceSessionRegistry::get_instance()->preload_session(model_path, Options(), mock_session);

    auto enhancer = create_enhancer(EnhancerType::GfpGan);
    ...
    // Set center pixel to 1.0f (so result should be 255)
    int center_idx = (h / 2) * w + (w / 2);
    output_data[0 * (h * w) + center_idx] = 1.0f; // R

...
protected:
    ...
    void SetUp() override {
        ...
        initializer->set_raw_data(std::string(reinterpret_cast<const char*>(data.data()), data.size() * sizeof(float)));
...
TEST_F(InSwapperTest, LoadModelAndSwapFace) {
    ...
    // Expectations
    // EXPECT_CALL(*mock_session, load_model(model_path, _)).Times(1); // Registry returns mock directly, skipping load_model

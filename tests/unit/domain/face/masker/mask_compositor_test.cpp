...
public:
    MOCK_METHOD(cv::Mat, create_region_mask,
                (const cv::Mat&, const std::unordered_set<FaceRegion>&), (override));
};
...
TEST_F(MaskCompositorTest, ComposeBoxMaskWithPadding) {
    ...
    // Note: pad_left, pad_right, pad_bottom default to 1 due to blur_area logic even if padding is
    // 0 So we need to crop left/right/bottom as well.
...
TEST_F(MaskCompositorTest, ComposeOcclusionMaskInvertsResult) {
    ...
    EXPECT_CALL(mock_occluder, create_occlusion_mask(_)).WillOnce(Return(occlusion_mask));
...
TEST_F(MaskCompositorTest, ComposeRegionMaskUsesDirectResult) {
    ...
    EXPECT_CALL(mock_region_masker, create_region_mask(_, _)).WillOnce(Return(region_mask));

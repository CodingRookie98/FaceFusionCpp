...
TEST_F(FairFaceTest, LoadModelAndClassify) {
    ...
    EXPECT_CALL(*mock_session, run(_)).WillRepeatedly(
        [&](const std::vector<Ort::Value>&) {
             ...
             outputs.push_back(Ort::Value::CreateTensor<int64_t>(
                 memory_info, const_cast<int64_t*>(&race_val), 1, 
                 const_cast<int64_t*>(shape.data()), shape.size()));

             // Gender
             outputs.push_back(Ort::Value::CreateTensor<int64_t>(
                 memory_info, const_cast<int64_t*>(&gender_val), 1, 
                 const_cast<int64_t*>(shape.data()), shape.size()));

             // Age
             outputs.push_back(Ort::Value::CreateTensor<int64_t>(
                 memory_info, const_cast<int64_t*>(&age_val), 1, 
                 const_cast<int64_t*>(shape.data()), shape.size()));
    ...
    // However, ONNX Runtime usually returns outputs based on the order of output nodes in the model.
    // In the real model, the order might be fixed. 
    // In my Mock, I return a vector. 
    ...
    // Inside the lambda: `const_cast<int64_t*>(&race_val)` takes the address of the lambda's member variable (the captured copy).
    // The lambda returns `outputs` vector containing `Ort::Value`s which point to the lambda's member variables.
    // When `run` returns, the lambda is destroyed?
    // `WillRepeatedly` stores the action (the lambda). The lambda itself persists as long as the expectation exists.
    // So the lambda object is alive.
    // BUT `outputs` vector is returned by value. The `Ort::Value`s are moved/copied.
    // The `Ort::Value` holds the pointer to the data.
    // The data is inside the lambda object.
    // So as long as the lambda object is alive, the pointer should be valid?
    // Yes, Google Mock actions are kept alive.
    ...
    // There isn't one that easily copies from a pointer without owning it unless we use a vector and keep it alive.

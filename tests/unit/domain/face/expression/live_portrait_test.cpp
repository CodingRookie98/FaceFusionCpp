...
TEST_F(LivePortraitTest, LoadModelAndRestoreExpression) {
    ...
    InferenceSessionRegistry::get_instance().preload_session(generator_path, Options(), generator_mock);
    ...
    EXPECT_CALL(*feature_mock, run(_)).WillRepeatedly(
        [=](const std::vector<Ort::Value>&) mutable {
             auto memory_info = Ort::MemoryInfo::CreateCpu(OrtArenaAllocator, OrtMemTypeDefault);
             std::vector<Ort::Value> outputs;
             outputs.push_back(Ort::Value::CreateTensor<float>(
                 memory_info, feature_data.data(), feature_data.size(), 
                 feature_output_shape.data(), feature_output_shape.size()));
             return outputs;
        }
    );
    ...
    EXPECT_CALL(*motion_mock, get_output_names()).WillRepeatedly(Return(std::vector<std::string>{"0", "1", "2", "3", "4", "5", "6"}));
    ...
    EXPECT_CALL(*motion_mock, run(_)).WillRepeatedly(
        [=](const std::vector<Ort::Value>&) {
             ...
             outputs.push_back(Ort::Value::CreateTensor<float>(memory_info, &scalar_val, 1, scalar_shape.data(), 1));
             outputs.push_back(Ort::Value::CreateTensor<float>(memory_info, &scalar_val, 1, scalar_shape.data(), 1));
             outputs.push_back(Ort::Value::CreateTensor<float>(memory_info, &scalar_val, 1, scalar_shape.data(), 1));
             static float scale_val = 1.0f;
             outputs.push_back(Ort::Value::CreateTensor<float>(memory_info, &scale_val, 1, scalar_shape.data(), 1));
             ...
             outputs.push_back(Ort::Value::CreateTensor<float>(memory_info, trans_val.data(), 3, trans_shape.data(), 2));

             // 5: Expression [21, 3]
             std::vector<int64_t> expr_shape = {1, 21, 3};
             static std::vector<float> expr_val(63, 0.0f);
             outputs.push_back(Ort::Value::CreateTensor<float>(memory_info, expr_val.data(), 63, expr_shape.data(), 3));

             // 6: Points [21, 3]
             static std::vector<float> pts_val(63, 0.0f);
             outputs.push_back(Ort::Value::CreateTensor<float>(memory_info, pts_val.data(), 63, expr_shape.data(), 3));
    ...
    EXPECT_CALL(*generator_mock, get_output_node_dims()).WillRepeatedly(Return(generator_output_dims));
    EXPECT_CALL(*generator_mock, get_input_names()).WillRepeatedly(Return(std::vector<std::string>{"feature_volume", "source", "target"}));
    ...
    EXPECT_CALL(*generator_mock, run(_)).WillRepeatedly(
        [=](const std::vector<Ort::Value>&) {
             auto memory_info = Ort::MemoryInfo::CreateCpu(OrtArenaAllocator, OrtMemTypeDefault);
             std::vector<Ort::Value> outputs;
             outputs.push_back(Ort::Value::CreateTensor<float>(

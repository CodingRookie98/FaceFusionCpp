...
protected:
    ...
    void TearDown() override { std::filesystem::remove_all(output_dir); }
...
TEST_F(PipelineRunnerMetricsTest, MetricsExport_SchemaVersion_Matches1_0) {
    ...
    for (int i = 0; i < 10; ++i) { collector.record_frame_completed(); }
...
TEST_F(PipelineRunnerMetricsTest, MetricsExport_StepLatency_RecordsMultipleSteps) {
    ...
    for (const auto& step : steps) {
        ...
        if (step["step_name"] == "face_enhance") { has_enhance = true; }
    }

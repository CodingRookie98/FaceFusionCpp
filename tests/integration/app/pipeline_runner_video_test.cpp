...
TEST_F(PipelineRunnerVideoTest, ProcessVideoEndToEnd) {
    ...
    task_config.io.output.suffix = "output"; // This will result in pipeline_runner_video_output.mp4
                                             // based on GenerateOutputPath logic check
    // Actually GenerateOutputPath logic: prefix + stem + suffix.
    // stem="slideshow_scaled".
    // filename = "pipeline_runner_video_" + "slideshow_scaled" + "output"
    // Output path expected: tests_output/pipeline_runner_video_slideshow_scaledoutput.mp4
    // Let's adjust expected output path to match logic:
    ...
    task_config.io.output.audio_policy = config::AudioPolicy::Copy; // Test audio copy path

...
FaceMaskerHub::~FaceMaskerHub() {
    ...
    for (auto val : m_maskers | std::views::values) { delete val; }
    m_maskers.clear();
}
...
FaceMaskerBase* FaceMaskerHub::get_masker(const FaceMaskerHub::Type& type, const Model& model) {
    ...
    if (m_maskers.contains(type)) {
        if (m_maskers[type] != nullptr) { return m_maskers[type]; }
    }
    ...
    if (masker != nullptr) { m_maskers[type] = masker; }
...
cv::Mat FaceMaskerHub::get_best_mask(const ArgsForGetBestMask& func_gbm_args) {
    ...
    if (func_gbm_args.faceMaskersTypes.contains(Type::Box) && func_gbm_args.boxSize.has_value()
        ...
        futures.emplace_back(ThreadPool::instance()->enqueue(
            create_static_box_mask, func_gbm_args.boxSize.value(),
            func_gbm_args.boxMaskBlur.value(), func_gbm_args.boxMaskPadding.value()));
    }

    if (func_gbm_args.faceMaskersTypes.contains(Type::Occlusion)
        && func_gbm_args.occlusionFrame.has_value() && func_gbm_args.occluder_model.has_value()) {
        futures.emplace_back(ThreadPool::instance()->enqueue([&] {
            return create_occlusion_mask(*func_gbm_args.occlusionFrame.value(),
                                         func_gbm_args.occluder_model.value());
        }));
    }

    if (func_gbm_args.faceMaskersTypes.contains(Type::Region)
        && func_gbm_args.regionFrame.has_value()) {
        futures.emplace_back(ThreadPool::instance()->enqueue([&] {
            return create_region_mask(*func_gbm_args.regionFrame.value(),
                                      func_gbm_args.parser_model.value(),
                                      func_gbm_args.faceMaskerRegions.value());
        }));
    }

    for (auto& future : futures) { masks.emplace_back(future.get()); }
...
cv::Mat FaceMaskerHub::create_occlusion_mask(const cv::Mat& cropVisionFrame,
    ...
    if (occluder_model != Model::xseg_1 && occluder_model != Model::xseg_2) {
        throw std::runtime_error("Occlusion model not supported.");
    }
    const auto faceMaskerOcclusion =
        dynamic_cast<Occlusion*>(get_masker(Type::Occlusion, occluder_model));
    return faceMaskerOcclusion->createOcclusionMask(cropVisionFrame);
}

cv::Mat FaceMaskerHub::create_region_mask(
    const cv::Mat& inputImage, const Model& parser_model,
    const std::unordered_set<FaceMaskerRegion::Region>& regions) {
    if (parser_model != Model::bisenet_resnet_18 && parser_model != Model::bisenet_resnet_34) {
        throw std::runtime_error("Region model not supported");
    }
    const auto faceMaskerRegion =
        dynamic_cast<FaceMaskerRegion*>(get_masker(Type::Region, parser_model));
    return faceMaskerRegion->createRegionMask(inputImage, regions);
}

cv::Mat FaceMaskerHub::create_static_box_mask(const cv::Size& cropSize, const float& faceMaskBlur,
    ...
    const int paddingTop =
        std::max(blurArea, static_cast<int>(cropSize.height * faceMaskPadding.at(0) / 100.0));
    const int paddingRight =
        std::max(blurArea, static_cast<int>(cropSize.width * faceMaskPadding.at(1) / 100.0));
    const int paddingBottom =
        std::max(blurArea, static_cast<int>(cropSize.height * faceMaskPadding.at(2) / 100.0));
    const int paddingLeft =
        std::max(blurArea, static_cast<int>(cropSize.width * faceMaskPadding.at(3) / 100.0));
    ...
    if (blurAmount > 0) { cv::GaussianBlur(boxMask, boxMask, cv::Size(0, 0), blurAmount * 0.25); }

    return boxMask;
}

cv::Mat FaceMaskerHub::get_best_mask(const std::vector<cv::Mat>& masks) {
    if (masks.empty()) { throw std::invalid_argument("The input vector is empty."); }

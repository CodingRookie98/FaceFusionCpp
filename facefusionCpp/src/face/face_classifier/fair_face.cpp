...
void FairFace::load_model(const std::string& modelPath, const Options& options) {
    ...
    m_inputHeight = m_input_node_dims[0][3];
...
FaceClassifierBase::Result FairFace::classify(const cv::Mat& image, const Face::Landmarks& faceLandmark5) {
    ...
    inputTensor.emplace_back(Ort::Value::CreateTensor<float>(m_memory_info->GetConst(),
...
AgeRange FairFace::categorizeAge(const int64_t& age_id) {
    AgeRange age{};

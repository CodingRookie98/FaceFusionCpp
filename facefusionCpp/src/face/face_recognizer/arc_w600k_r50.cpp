...
ArcW600kR50::ArcW600kR50(const std::shared_ptr<Ort::Env>& env) : FaceRecognizerBase(env) {}

void ArcW600kR50::load_model(const std::string& modelPath, const Options& options) {
    FaceRecognizerBase::load_model(modelPath, options);
    m_inputWidth = static_cast<int>(m_input_node_dims[0][2]);
    m_inputHeight = static_cast<int>(m_input_node_dims[0][3]);
}

std::vector<float> ArcW600kR50::preProcess(const cv::Mat& visionFrame,
                                           const Face::Landmarks& faceLandmark5_68) const {
    std::vector<cv::Point2f> warpTemplate =
        face_helper::getWarpTemplate(face_helper::WarpTemplateType::Arcface_112_v2);
    cv::Mat cropVisionFrame;
    std::tie(cropVisionFrame, std::ignore) = face_helper::warpFaceByFaceLandmarks5(
        visionFrame, faceLandmark5_68, warpTemplate, cv::Size(112, 112));
...
std::array<std::vector<float>, 2> ArcW600kR50::recognize(const cv::Mat& visionFrame,
    ...
    for (int i = 0; i < lenFeature; i++) { normedEmbedding.at(i) = embedding.at(i) / (float)norm; }

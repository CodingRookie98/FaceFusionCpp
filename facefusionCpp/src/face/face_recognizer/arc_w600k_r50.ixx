...
public:
    ...
    std::array<std::vector<float>, 2> recognize(const cv::Mat& visionFrame,
                                                const Face::Landmarks& faceLandmark5) override;
    void load_model(const std::string& modelPath, const Options& options) override;

private:
    std::vector<float> preProcess(const cv::Mat& visionFrame,

...
FaceDetectorHub::Detect(const cv::Mat& image, const Options& options) {
    ...
    if (options.types.contains(Type::Retina)) {
        ...
        if (retina != nullptr) {
            if (options.angle > 0) {
                futures.emplace_back(ThreadPool::instance()->enqueue(std::bind(&Retina::detect_rotated_faces, retina, image, options.size, options.angle, options.min_score)));
            } else {
                futures.emplace_back(ThreadPool::instance()->enqueue(std::bind(&Retina::DetectFaces, retina, image, options.size, options.min_score)));
            }
        }
    }
    ...
    if (options.types.contains(Type::Scrfd)) {
        ...
        if (scrfd != nullptr) {
            if (options.angle > 0) {
                futures.emplace_back(ThreadPool::instance()->enqueue(std::bind(&Scrfd::detect_rotated_faces, scrfd, image, options.size, options.angle, options.min_score)));
            } else {
                futures.emplace_back(ThreadPool::instance()->enqueue(std::bind(&Scrfd::DetectFaces, scrfd, image, options.size, options.min_score)));
            }
        }
    }
    ...
    if (options.types.contains(Type::Yolo)) {
        ...
        if (yolo != nullptr) {
            if (options.angle > 0) {
                futures.emplace_back(ThreadPool::instance()->enqueue(std::bind(&Yolo::detect_rotated_faces, yolo, image, options.size, options.angle, options.min_score)));
            } else {
                futures.emplace_back(ThreadPool::instance()->enqueue(std::bind(&Yolo::DetectFaces, yolo, image, options.size, options.min_score)));
            }
        }
    }
...
std::shared_ptr<FaceDetectorBase> FaceDetectorHub::GetDetector(const Type& type) {
    ...
    if (type == Type::Retina) {
        ...
        detector->load_model(modelManager->get_model_path(Model::Face_detector_retinaface), inference_session_options_);
    } else if (type == Type::Scrfd) {
        detector = std::make_shared<Scrfd>(env_);
        detector->load_model(modelManager->get_model_path(Model::Face_detector_scrfd), inference_session_options_);
    } else if (type == Type::Yolo) {
        detector = std::make_shared<Yolo>(env_);
        detector->load_model(modelManager->get_model_path(Model::Face_detector_yoloface), inference_session_options_);
    }

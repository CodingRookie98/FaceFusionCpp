...
void Scrfd::load_model(const std::string& modelPath, const Options& options) {
    FaceDetectorBase::load_model(modelPath, options);
...
Scrfd::DetectFaces(const cv::Mat& visionFrame, const cv::Size& faceDetectorSize,
    ...
    for (size_t index = 0; index < m_featureStrides.size(); ++index) {
        ...
        auto* pdataScore = ortOutputs[index].GetTensorMutableData<float>();
        ...
        for (const auto& keepIndex : keepIndices) {
            ...
            resultScores.emplace_back(float{*(pdataScore + keepIndex)});
        }
    }
    Result result;
    result.boxes = std::move(resultBoundingBoxes);
...
Scrfd::preProcess(const cv::Mat& visionFrame, const cv::Size& faceDetectorSize) {
    ...
    const cv::Mat tempVisionFrame = vision::resize_frame(visionFrame, cv::Size(faceDetectorWidth, faceDetectorHeight));

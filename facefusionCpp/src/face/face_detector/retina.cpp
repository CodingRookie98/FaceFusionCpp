...
void Retina::load_model(const std::string& modelPath, const Options& options) {
...
Retina::preProcess(const cv::Mat& visionFrame, const cv::Size& faceDetectorSize) {
    ...
    const auto tempVisionFrame = vision::resize_frame(visionFrame, cv::Size(faceDetectorWidth, faceDetectorHeight));

...
void Yolo::load_model(const std::string& modelPath, const Options& options) {
    FaceDetectorBase::load_model(modelPath, options);
    input_height_ = m_input_node_dims[0][2];
    input_width_ = m_input_node_dims[0][3];
}
...
Yolo::preProcess(const cv::Mat& visionFrame, const cv::Size& faceDetectorSize) {
    ...
    cv::Mat tempVisionFrame = vision::resize_frame(visionFrame, faceDetectorSize);

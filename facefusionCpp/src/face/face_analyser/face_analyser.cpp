...
Face FaceAnalyser::GetAverageFace(const std::vector<Face>& faces) {
    ...
    if (faces.size() > 1) {
        std::vector<Face::Embeddings> embeddings, normEmbeddings;
        for (auto& face : faces) {
        ...
        averageFace.m_normedEmbedding = face_helper::calcAverageEmbedding(normEmbeddings);
    }
...
FaceAnalyser::CreateFaces(const cv::Mat& visionFrame, const std::vector<Face::BBox>& bBoxes,
    ...
    for (const auto& index : keepIndices) {
        ...
        tempFace.m_landmark68From5 = ExpandFaceLandmarks68From5(tempFace.m_landmark5);
        ...
        if (options.faceLandMarkerOptions.minScore > 0) {
            ...
            if (tempFace.m_landmarkerScore < options.faceLandMarkerOptions.minScore) {
                ...
                for (int angle = 90; angle < 360; angle += 90) {
                    ...
                    std::tie(tempFace.m_landmark68, tempFace.m_landmarkerScore) = faceLandMarkerHub_.detectLandmark68(visionFrame, tempFace.m_bBox, faceLandmarkerOptions);
                    if (tempFace.m_landmarkerScore > options.faceLandMarkerOptions.minScore) {
                        tempFace.m_landMark5From68 = face_helper::convertFaceLandmark68To5(tempFace.m_landmark68);
                        isFound = true;
                        break;
                    }
                }
                if (!isFound) {
                    tempFace.m_landmark68 = tempFace.m_landmark68From5;
                    tempFace.m_landMark5From68 = tempFace.m_landmark5;
        ...
        std::array<Face::Embeddings, 2> embeddingAndNormedEmbedding = this->CalculateEmbedding(visionFrame, tempFace.m_landMark5From68, options.faceRecognizerType);
        ...
        std::tie(tempFace.m_gender, tempFace.m_age, tempFace.m_race) = this->ClassifyFace(visionFrame, tempFace.m_landMark5From68);
...
std::tuple<Face::Gender, Face::Age, Face::Race> FaceAnalyser::ClassifyFace(const cv::Mat& visionFrame, const Face::Landmarks& faceLandmarks5) {

...
FaceDetectorYolo::FaceDetectorYolo(const std::shared_ptr<Ort::Env> &env, const std::string &modelPath) :
...
FaceDetectorYolo::preProcess(const cv::Mat &visionFrame, const cv::Size &faceDetectorSize) {
    const int faceDetectorHeight = faceDetectorSize.height;
    const int faceDetectorWidth = faceDetectorSize.width;

    cv::Mat tempVisionFrame = Ffc::Vision::resizeFrameResolution(visionFrame, faceDetectorSize);
...
FaceDetectorYolo::Result
FaceDetectorYolo::detectFaces(const cv::Mat &visionFrame, const cv::Size &faceDetectorSize,

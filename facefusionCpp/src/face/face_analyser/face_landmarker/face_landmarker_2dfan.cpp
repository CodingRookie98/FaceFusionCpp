...
std::tuple<Face::Landmark, float> FaceLandmarker2dfan::detect(const cv::Mat &visionFrame, const Face::BBox &bBox) {
...
FaceLandmarker2dfan::FaceLandmarker2dfan(const std::shared_ptr<Ort::Env> &env, const std::string &modelPath) :
...
std::tuple<std::vector<float>, cv::Mat> FaceLandmarker2dfan::preProcess(const cv::Mat &visionFrame, const Face::BBox &bBox) {

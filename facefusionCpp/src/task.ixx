...
public:
    ...
    struct VideoInfo {
        ...
        VideoEncoder video_encoder{VideoEncoder::libx264}; // 指定视频编码器。默认: libx264, 选择: libx264 libx265 libvpx-vp9 h264_nvenc hevc_nvenc h264_amf hevc_amf, 示例: libx265
        VideoPreset video_preset{VideoPreset::veryslow};   // 输出视频编码器预设。默认: veryslow, 选择: ultrafast superfast veryfast faster fast medium slow slower veryslow, 示例: faster
        unsigned short video_quality{100};                 // 设置输出的视频质量。默认: 80, 范围: 0 to 100 at 1, 示例: 60
        AudioEncoder audio_encoder{AudioEncoder::aac};     // 设置音频编码器。默认: aac, 选择: aac libmp3lame libopus libvorbis, 示例: libmp3lame
        bool skip_audio{false};                            // 设置是否跳过音频处理。默认: false, 选择: true false, 示例: true
        ImageFormat frame_format{ImageFormat::PNG};        // 设置提取的视频帧格式。默认: png, 选择: png jpg bmp
    };
    VideoInfo video;
    ...
    struct FaceMasker {
        ...
        std::unordered_set<face_masker::FaceMaskerRegion::Region> mask_regions{face_masker::FaceMaskerRegion::getAllRegions()};
    };

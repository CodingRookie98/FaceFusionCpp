...
cv::Mat InSwapper::swapFace(const InSwapperInput &input) {
    ...
    for (size_t i = 0; i < croppedTargetFrames.size(); ++i) {
        ...
        func_gbm_args.occlusionFrame = &croppedTargetFrames[i];
        func_gbm_args.regionFrame = &croppedResultFrames[i];
        func_gbm_args.faceMaskerRegions = input.faceMaskerRegions;

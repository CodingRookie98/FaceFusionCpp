...
RealEsrGan::RealEsrGan(const std::shared_ptr<Ort::Env>& env) : InferenceSession(env) {}
...
cv::Mat RealEsrGan::enhanceFrame(const RealEsrGanInput& input) const {
    ...
    auto [tileVisionFrames, padWidth, pidHeight] =
        vision::create_tile_frames(*input.target_frame, m_tileSize);
    ...
    for (size_t i = 0; i < tileVisionFrames.size(); i++) {
        ...
        const int outputWidth =
            static_cast<int>(outputTensor[0].GetTensorTypeAndShapeInfo().GetShape()[2]);
        const int outputHeight =
            static_cast<int>(outputTensor[0].GetTensorTypeAndShapeInfo().GetShape()[3]);
        cv::Mat outputImage = get_output_data(outputData, cv::Size(outputWidth, outputHeight));
        tileVisionFrames[i] = std::move(outputImage);
    }

    cv::Mat outputImage = vision::merge_tile_frames(
        tileVisionFrames, tempWidth * m_modelScale, tempHeight * m_modelScale,

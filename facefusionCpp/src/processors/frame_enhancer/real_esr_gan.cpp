...
RealEsrGan::RealEsrGan(const std::shared_ptr<Ort::Env> &env, const std::string &modelPath) :
    FrameEnhancerBase(env, modelPath) {
}

cv::Mat RealEsrGan::processFrame(const ProcessorBase::InputData *inputData) {
...
std::unordered_set<ProcessorBase::InputDataType> RealEsrGan::getInputDataTypes() {
    return {ProcessorBase::InputDataType::TargetFrame};
}

cv::Mat RealEsrGan::enhanceFrame(const cv::Mat &frame) const {

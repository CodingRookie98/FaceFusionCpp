...
std::string LivePortrait::get_processor_name() const {
    return "ExpressionRestorer.LivePortrait";
}

void LivePortrait::loadModel(const std::string& featureExtractorPath,
                             const std::string& motionExtractorPath,
                             const std::string& generatorPath,
                             const ai::InferenceSession::Options& options) {
    m_featureExtractor.load_model(featureExtractorPath, options);
    m_motionExtractor.load_model(motionExtractorPath, options);
    m_generator.load_model(generatorPath, options);
...
cv::Mat LivePortrait::restoreExpression(const LivePortraitInput& input) {
    ...
    if (!m_featureExtractor.is_model_loaded() || !m_motionExtractor.is_model_loaded()
        || !m_generator.is_model_loaded()) {
        throw std::runtime_error(
            std::format("File: {}, Line: {}, Error: models is not loaded!", __FILE__, __LINE__));
    }
    ...
    std::vector<cv::Mat> croppedSourceFrames, croppedTargetFrames, affineMatrices, bestMasks,
        restoredFrames;
    for (size_t i = 0; i < input.source_faces_5_landmarks.size(); ++i) {
        ...
        std::tie(sourceCroppedFrame, std::ignore) =
            face_helper::warpFaceByFaceLandmarks5(*input.source_frame, source_faces_5_landmark,
                                                  m_warpTemplateType, m_generatorOutputSize);
        std::tie(targetCropFrame, affineMat) =
            face_helper::warpFaceByFaceLandmarks5(*input.target_frame, target_faces_5_landmark,
                                                  m_warpTemplateType, m_generatorOutputSize);
    ...
    for (auto& croppedTargetFrame : croppedTargetFrames) {
        ...
        bestMasks.emplace_back(m_faceMaskerHub->get_best_mask(args_for_get_best_mask));
    }

    if (croppedSourceFrames.size() != croppedTargetFrames.size()
        || croppedSourceFrames.size() != bestMasks.size()
        || croppedSourceFrames.size() != affineMatrices.size()) {
        return input.target_frame->clone();
    }
    ...
    for (size_t i = 0; i < croppedTargetFrames.size(); ++i) {
        resultFrame =
            face_helper::pasteBack(resultFrame, restoredFrames[i], bestMasks[i], affineMatrices[i]);
    }
    return resultFrame;
}

cv::Mat LivePortrait::applyRestore(const cv::Mat& croppedSourceFrame,
    ...
    std::vector<std::vector<float>> targetMotion =
        m_motionExtractor.extractMotion(croppedTargetFrame);

    std::vector<float> featureVolume = featureVolumeFu.get();
    std::vector<std::vector<float>> sourceMotion = sourceMotionFu.get();

    cv::Mat rotationMat =
        createRotationMat(targetMotion[0][0], targetMotion[1][0], targetMotion[2][0]);
    std::vector<float> sourceExpression = std::move(sourceMotion[5]),
                       targetExpression = std::move(targetMotion[5]);
    ...
    sourceExpressionMat =
        sourceExpressionMat * m_restoreFactor + targetExpressionMat * (1 - m_restoreFactor);
    sourceExpressionMat = limitExpression(sourceExpressionMat);

    cv::Mat targetTranslationMat(21, 3, CV_32FC1);
    for (int i = 0; i < 21; ++i) {
        for (int j = 0; j < 3; ++j) { targetTranslationMat.at<float>(i, j) = targetMotion[4][j]; }
    }
    float targetScale = targetMotion[3][0];

    cv::Mat targetMotionPoints(21, 3, CV_32FC1, targetMotion[6].data());
    cv::Mat sourceMotionPoints =
        targetScale * (targetMotionPoints * rotationMat.t() + sourceExpressionMat)
        + targetTranslationMat;
    targetMotionPoints = targetScale * (targetMotionPoints * rotationMat.t() + targetExpressionMat)
                       + targetTranslationMat;

    std::vector<float> sourceMotionPointsData, targetMotionPointsData;
    sourceMotionPointsData.assign(sourceMotionPoints.begin<float>(),
                                  sourceMotionPoints.end<float>());
    targetMotionPointsData.assign(targetMotionPoints.begin<float>(),
                                  targetMotionPoints.end<float>());

    cv::Mat result =
        m_generator.generateFrame(featureVolume, sourceMotionPointsData, targetMotionPointsData);
...
std::vector<float> LivePortrait::FeatureExtractor::extractFeature(const cv::Mat& frame) const {
    ...
    const int &width = m_input_node_dims[0][2], &height = m_input_node_dims[0][3];
    ...
    inputTensors.emplace_back(Ort::Value::CreateTensor<float>(
        m_memory_info->GetConst(), inputImageData.data(), inputImageData.size(), inputShape.data(),
        inputShape.size()));
...
std::vector<std::vector<float>> LivePortrait::MotionExtractor::extractMotion(
    ...
    for (size_t i = 0; i < m_output_names.size(); ++i) {
        auto* outputData = outputTensor[i].GetTensorMutableData<float>();
        if (i == 0 || i == 1 || i == 2 || i == 3) {
            outputDataVec.emplace_back(outputData, outputData + 1);
        }
        if (i == 4) { outputDataVec.emplace_back(outputData, outputData + 1 * 3); }
        if (i == 5 || i == 6) { outputDataVec.emplace_back(outputData, outputData + 1 * 21 * 3); }
    }

    return outputDataVec;
}

LivePortrait::Generator::Generator(const std::shared_ptr<Ort::Env>& env) : InferenceSession(env) {}
...
cv::Mat LivePortrait::Generator::generateFrame(std::vector<float>& featureVolume,
    ...
    for (const auto& name : m_input_names) {
        ...
        if (nameStr == "feature_volume") {
            inputTensors.emplace_back(Ort::Value::CreateTensor<float>(
                m_memory_info->GetConst(), featureVolume.data(), featureVolume.size(),
                inputFeatureShape.data(), inputFeatureShape.size()));
        }
        if (nameStr == "source") {
            inputTensors.emplace_back(Ort::Value::CreateTensor<float>(
                m_memory_info->GetConst(), sourceMotionPoints.data(), sourceMotionPoints.size(),
                inputMotionShape.data(), inputMotionShape.size()));
        }
        if (nameStr == "target") {
            inputTensors.emplace_back(Ort::Value::CreateTensor<float>(
                m_memory_info->GetConst(), targetMotionPoints.data(), targetMotionPoints.size(),
                inputMotionShape.data(), inputMotionShape.size()));
        }
    }
    ...
    channelMats[0] =
        cv::Mat(outputHeight, outputWidth, CV_32FC1, outputData + 2 * channelStep); // B
...
cv::Mat LivePortrait::createRotationMat(float pitch, float yaw, float roll) {
    ...
    const cv::Mat Ry =
        (cv::Mat_<float>(3, 3) << cos(yaw), 0, sin(yaw), 0, 1, 0, -sin(yaw), 0, cos(yaw));

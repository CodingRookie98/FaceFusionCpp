...
Core::Core(const CoreOptions& options) : m_processor_hub(options.inference_session_options) {
...
bool Core::run_task(Task task) {
    for (const auto& [type, model, param] : task.processors_info) {
        if (type == ProcessorMajorType::FaceSwapper || type == ProcessorMajorType::FaceEnhancer
            || type == ProcessorMajorType::ExpressionRestorer) {
            if (m_face_analyser == nullptr) {
                m_face_analyser =
                    std::make_shared<FaceAnalyser>(m_env, m_core_options.inference_session_options);
            }
        }
    }
...
bool Core::ProcessVideo(CoreTask core_task) {
    if (core_task.target_paths.size() > 1) {
        m_logger->warn(std::format(
            "{}: Only one target video is supported. Only the first video is processed this time. \n{}",
            __FUNCTION__, core_task.target_paths.front()));
    }

    std::string& videoPath = core_task.target_paths.front();

    std::string audiosDir =
        file_system::absolute_path(file_system::parent_path(videoPath) + "/audios");
    ...
    std::string videoFramesOutputDir = file_system::absolute_path(
        file_system::parent_path(videoPath) + "/" + file_system::get_base_name(videoPath));
    ...
    std::string outputVideo_NA_Path = file_system::parent_path(videoPath) + "/"
                                    + file_system::get_base_name(videoPath) + "_processed_NA"
                                    + file_system::get_file_ext(videoPath);
    Logger::get_instance()->info("[Core] Images to video : "
                                 + file_system::absolute_path(outputVideo_NA_Path));
...
bool Core::ProcessVideoInSegments(CoreTask core_task) {
    ...
    Logger::get_instance()->info(
        std::format("[Core] Divide the video into segments of {} seconds each....",
                    core_task.video_segment_duration.value()));
    if (!cutVideoIntoSegments(videoPath, videoSegmentsDir, core_task.video_segment_duration.value(),
                              videoSegmentPattern)) {
    ...
    std::unordered_set<std::string> videoSegmentsPathsSet =
        file_system::list_files(videoSegmentsDir);
    videoSegmentsPathsSet = filterVideoPaths(videoSegmentsPathsSet);

    std::vector<std::string> processedVideoSegmentsPaths;
    std::string processedVideoSegmentsDir =
        file_system::parent_path(videoPath) + "/videoSegments_processed";
    std::vector<std::string> videoSegmentsPathsVec(videoSegmentsPathsSet.begin(),
                                                   videoSegmentsPathsSet.end());
    std::ranges::sort(videoSegmentsPathsVec,
    ...
    for (size_t segmentIndex = 0; segmentIndex < videoSegmentsPathsVec.size(); ++segmentIndex) {
        ...
        std::string outputVideoSegmentPath = file_system::absolute_path(
            processedVideoSegmentsDir + "/" + file_system::get_file_name(videoSegmentPath));
        ...
        m_logger->info(std::format("[Core] Processing video segment {}/{}", segmentIndex + 1,
                                   videoSegmentsPathsSet.size()));
        if (!ProcessVideo(tmpCoreRunOptions)) {
            m_logger->error(
                std::format("[Core] Failed to process video segment: {}", videoSegmentPath));
            return false;
        }
    ...
    for (size_t i = 0; i < core_task.target_paths.size(); ++i) {
        ...
        if (core_task.video_segment_duration.value() > 0) {
            if (ProcessVideoInSegments(tmpCoreRunOptions)) {
                m_logger->info(std::format("[Core] Video processed successfully. Output path: {} ",
                                           core_task.output_paths[i]));
            } else {
                isAllSuccess = false;
                m_logger->error(
                    std::format("[Core] Video {} processed failed.", core_task.target_paths[i]));
            }
        } else {
            if (ProcessVideo(tmpCoreRunOptions)) {
                m_logger->info(std::format("[Core] Video processed successfully. Output path: {} ",
        ...
        if (autoRemoveTarget) { file_system::remove_file(core_task.target_paths[i]); }
    }
    return isAllSuccess;
}
...
bool Core::Run(CoreTask core_task) {
    ...
    if (core_task.processor_minor_types.contains(ProcessorMajorType::FaceSwapper)
        && core_task.processor_minor_types.at(ProcessorMajorType::FaceSwapper)
               == ProcessorMinorType::FaceSwapper_InSwapper) {
        std::unordered_set<std::string> sourceImgPaths(core_task.source_paths->begin(),
                                                       core_task.source_paths->end());
        core_task.source_average_face_id = {crypto::combined_sha1(sourceImgPaths)};
        core_task.source_average_face =
            std::make_shared<Face>(core_task.ProcessSourceAverageFace(m_face_analyser));
    }
    ...
    for (size_t i = 0; i < core_task.target_paths.size(); ++i) {
        ...
        if (vision::is_image(targetPath)) {
            ...
            const std::string file_symlink_path =
                tmpPath + "/videos/" + file_system::get_file_name(targetPath);
            file_system::create_dir(tmpPath + "/videos");
            try {
                std::filesystem::create_symlink(targetPath, file_symlink_path);
            } catch (std::filesystem::filesystem_error& e) {
                m_logger->error(
                    std::format("[Core::Run] Create symlink failed! Error: {}", e.what()));
    ...
    if (!tmpTargetImgPaths.empty() && tmpTargetImgPaths.size() == outputImgPaths.size()) {
        ...
        if (!ProcessImages(tmpRunOptions)) { imagesAllOK = false; }
    ...
    if (!tmpTargetVideoPaths.empty() && tmpTargetVideoPaths.size() == outputVideoPaths.size()) {
        ...
        if (!ProcessVideos(tmpRunOptions, true)) { videosAllOK = false; }
    }
...
bool Core::ProcessImages(CoreTask core_task) {
    ...
    if (core_task.processor_model.contains(ProcessorMajorType::FaceSwapper)
        && core_task.processor_minor_types[ProcessorMajorType::FaceSwapper]
               == ProcessorMinorType::FaceSwapper_InSwapper) {
    ...
    if (core_task.processor_list.front() == ProcessorMajorType::ExpressionRestorer) {
        if (!core_task.source_paths.has_value()) {
            ...
            m_logger->error(std::string(__FUNCTION__)
                            + " target_paths and source_paths size mismatch");
        }
    }
    ...
    if (core_task.processor_minor_types.contains(ProcessorMajorType::ExpressionRestorer)) {
        if (core_task.processor_list.front() != ProcessorMajorType::ExpressionRestorer) {
            for (const auto& path : core_task.target_paths) {
                const std::string tmpSourcePath = file_system::parent_path(path) + "/"
                                                + file_system::get_base_name(path) + "_original"
                                                + file_system::get_file_ext(path);
                originalTargetPaths.emplace_back(tmpSourcePath);
            }
            file_system::copy_files(core_task.target_paths, originalTargetPaths);
        } else {
            if (!core_task.source_paths.has_value()) { core_task.source_paths = {{}}; }
    ...
    for (const auto& type : core_task.processor_list) {
        if (type == ProcessorMajorType::ExpressionRestorer) {
            if (originalTargetPaths.size() != core_task.target_paths.size()) {
                m_logger->error(
                    ...
                    + std::format(" source Paths size is {}, but target paths size is {}!",
                                  originalTargetPaths.size(), core_task.target_paths.size()));
                return false;
            }
        ...
        auto func_to_process = [&](const size_t index) -> bool {
            if (type == ProcessorMajorType::FaceSwapper) {
                return SwapFace(
                    core_task.GetFaceSwapperInput(index, m_face_analyser),
            ...
            if (type == ProcessorMajorType::FaceEnhancer) {
                return EnhanceFace(
                    core_task.GetFaceEnhancerInput(index, m_face_analyser),
                    core_task.output_paths.at(index),
            ...
            if (type == ProcessorMajorType::ExpressionRestorer) {
                ...
                return RestoreExpression(
                    tmpCoreRunOptions.GetExpressionRestorerInput(index, index, m_face_analyser),
                    core_task.output_paths.at(index),
                    get_expression_restorer_type(core_task.processor_minor_types[type]).value());
            }
            if (type == ProcessorMajorType::FrameEnhancer) {
                return EnhanceFrame(
                    core_task.GetFrameEnhancerInput(index), core_task.output_paths.at(index),
        ...
        if (core_task.show_progress_bar) {
            ...
            if (type == ProcessorMajorType::FaceSwapper) {
                processor_name =
                    m_processor_hub.getProcessorPool()
                        .get_face_swapper(
                            get_face_swapper_type(core_task.processor_minor_types[type]).value(),
                            core_task.processor_model[type])
                        ->get_processor_name();
            ...
            if (type == ProcessorMajorType::ExpressionRestorer) {
                processor_name =
                    m_processor_hub.getProcessorPool()
                        .get_expression_restorer(
                            get_expression_restorer_type(core_task.processor_minor_types[type])
                                .value())
                        ->get_processor_name();
            }
            if (type == ProcessorMajorType::FrameEnhancer) {
        ...
        if (m_observer) { m_observer->on_start(core_task.target_paths.size()); }
        ...
        for (size_t k = 0; k < futures.size(); ++k) {
            ...
            while (!result_obtained) {
                ...
                const std::future_status status =
                    futures[k].wait_for(std::chrono::milliseconds(100));
            ...
            if (!is_success) {
                ...
                m_logger->error(std::format("[{}] Failed to write image: {}", processor_name,
                                            core_task.output_paths[k]));
            }

            if (core_task.show_progress_bar) {
                progress_bar->set_postfix_text(std::format("{}/{}", k + 1, futures.size()));
                const int progress = static_cast<int>(std::floor(static_cast<float>(k + 1) * 100.0f)
                                                      / static_cast<float>(futures.size()));
                progress_bar->set_progress(progress);
            }

            if (m_observer) {
                m_observer->on_progress(k + 1,
                                        std::format("Processing {}/{}", k + 1, futures.size()));
            }
        }

        if (core_task.show_progress_bar) { ProgressBar::show_console_cursor(true); }
        if (!is_all_success) {
            m_logger->error(
                std::format("[{}] Some images failed to process or write.", processor_name));
        }
        ...
        if (m_core_options.memory_options.processor_memory_strategy
            == CoreOptions::MemoryStrategy::Strict) {
...
bool Core::SwapFace(const FaceSwapperInput& face_swapper_input, const std::string& output_path,
    ...
    if (type == FaceSwapperType::InSwapper) {
        ...
        swapped_frame =
            m_processor_hub.swapFace(FaceSwapperType::InSwapper, model, face_swapper_input);
    }

    if (!swapped_frame.empty()) {
        std::ignore =
            ThreadPool::instance()->enqueue(vision::write_image, swapped_frame, output_path);
    } else {
        m_logger->error(
            std::format("Swap face failed! Result frame is empty. And target_frame is {}!",
                        target_frame->empty() ? "empty" : "not empty"));
        std::ignore =
            ThreadPool::instance()->enqueue(vision::write_image, *target_frame, output_path);
    }

    return true;
}

bool Core::EnhanceFace(const FaceEnhancerInput& face_enhancer_input, const std::string& output_path,
    ...
    if (type == FaceEnhancerType::CodeFormer) {
        ...
        enhanced_frame =
            m_processor_hub.enhanceFace(FaceEnhancerType::CodeFormer, model, face_enhancer_input);
    }
    if (type == FaceEnhancerType::GFP_GAN) {
        target_frame = face_enhancer_input.gfp_gan_input->target_frame;
        enhanced_frame =
            m_processor_hub.enhanceFace(FaceEnhancerType::GFP_GAN, model, face_enhancer_input);
    }

    if (!enhanced_frame.empty()) {
        std::ignore =
            ThreadPool::instance()->enqueue(vision::write_image, enhanced_frame, output_path);
    } else {
        m_logger->error(
            std::format("Enhance face failed! Result frame is empty. And target_frame is {}!",
                        target_frame->empty() ? "empty" : "not empty"));
        std::ignore =
            ThreadPool::instance()->enqueue(vision::write_image, *target_frame, output_path);
    }
...
bool Core::RestoreExpression(const ExpressionRestorerInput& expression_restorer_input,
    ...
    if (!restored_frame.empty()) {
        std::ignore =
            ThreadPool::instance()->enqueue(vision::write_image, restored_frame, output_path);
    } else {
        m_logger->error(
            std::format("Restore expression failed! Result frame is empty. And target_frame is {}!",
                        target_frame->empty() ? "empty" : "not empty"));
        std::ignore =
            ThreadPool::instance()->enqueue(vision::write_image, *target_frame, output_path);
    }
...
bool Core::EnhanceFrame(const FrameEnhancerInput& frame_enhancer_input,
    ...
    if (type == FrameEnhancerType::Real_esr_gan) {
        ...
        enhanced_frame = m_processor_hub.enhanceFrame(FrameEnhancerType::Real_esr_gan, model,
    ...
    if (type == FrameEnhancerType::Real_hat_gan) {
        ...
        enhanced_frame = m_processor_hub.enhanceFrame(FrameEnhancerType::Real_hat_gan, model,
                                                      frame_enhancer_input);
    }
    ...
    if (!enhanced_frame.empty()) {
        ...
        m_logger->error(
            std::format("Enhance frame failed! Result frame is empty. And target_frame is {}!",
                        target_frame->empty() ? "empty" : "not empty"));
        std::ignore =
            ThreadPool::instance()->enqueue(vision::write_image, *target_frame, output_path);
    }

...
export namespace foundation::media::vision {
    ...
    export std::tuple<std::vector<cv::Mat>, int, int>
    create_tile_frames(const cv::Mat& vision_frame, const std::vector<int>& size);

    export cv::Mat merge_tile_frames(const std::vector<cv::Mat>& tile_frames, int temp_width, int temp_height,
    ...
    export std::unordered_set<std::string> filter_image_paths(const std::unordered_set<std::string>& paths);

    export bool copy_image(const std::string& image_path, const std::string& destination, const cv::Size& size = cv::Size(0, 0));

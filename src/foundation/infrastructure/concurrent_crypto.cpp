...
#include <algorithm>
#include <iostream>
// Re-include OpenSSL only if we need it for combination, but we can reuse crypto module if we
// implemented a string hasher there. But wait, crypto module only exports sha1(file) and
// combined_sha1(files). If we want to hash the combined hashes, we need a string hasher. I'll
// re-include OpenSSL here just for the string hashing part for combination.
...
std::string combined_sha1(const std::unordered_set<std::string>& file_paths) {
    ...
    for (const auto& hash : hashes) { EVP_DigestUpdate(mdctx, hash.c_str(), hash.size()); }

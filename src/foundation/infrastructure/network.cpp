...
size_t header_callback(char* buffer, size_t size, size_t nitems, void* userdata) {
    ...
    if (header.find("Content-Length:") == 0) {
        ...
        if (colon_pos != std::string::npos) { *content_length = header.substr(colon_pos + 1); }
    }
    return total_size;
}
...
std::vector<bool> batch_download(const std::vector<std::string>& urls,
    ...
    for (const auto& url : urls) {
        ...
        } catch (const std::exception&) { results.push_back(false); }
    }
...
bool is_downloaded(const std::string& url, const std::string& file_path) {
    if (url.empty()) { throw std::invalid_argument("URL cannot be empty"); }
    if (file_path.empty()) { throw std::invalid_argument("File path cannot be empty"); }
...
std::string get_file_name_from_url(const std::string& url) {
    ...
    if (last_slash_pos == std::string::npos) { return "downloaded_file"; }

    std::string file_name = url_without_query.substr(last_slash_pos + 1);
    if (file_name.empty()) { return "downloaded_file"; }

...
namespace domain::face::detector {
    ...
    DetectionResults Scrfd::detect(const cv::Mat& visionFrame) {
        ...
        for (size_t index = 0; index < m_featureStrides.size(); ++index) {
            int size = ortOutputs[index].GetTensorTypeAndShapeInfo().GetShape()[0]; // Batch * Height * Width * Anchors? No, usually Flattened?
            ...
            // pdataLandmark = ortOutputs[index + 2 * m_featureMapChannel].GetTensorMutableData<float>();
        }
        */
        ...
        for (size_t index = 0; index < m_featureStrides.size(); ++index) {
             ...
             int numAnchors = static_cast<int>(scoreInfo.GetElementCount()); // Assuming shape is [N, H*W*A, 1] or similar flattened
             ...
             const float* pdataLandmark = ortOutputs[index + 2 * featureMapChannel].GetTensorData<float>();
        ...
        // NMS
        auto keepIndices = domain::face::helper::apply_nms(boundingBoxesRaw, confidencesRaw, 0.4f); // 0.4 NMS threshold

...
namespace domain::face::selector {
    ...
    namespace {
        ...
        std::vector<Face> filter_by_race(std::vector<Face> faces, const std::unordered_set<domain::common::types::Race>& races) {
            if (faces.empty()) {
                return faces;
            }
            if (races.size() == ALL_RACES.size()) {
                return faces;
            }

            std::erase_if(faces, [&](const Face& face) {
                return !races.contains(face.race());
            });
            return faces;
        }

        std::vector<Face> filter_by_gender(std::vector<Face> faces, const std::unordered_set<domain::common::types::Gender>& genders) {
            if (faces.empty()) {
                return faces;
            }
            if (genders.size() == ALL_GENDERS.size()) {
                return faces;
            }

            std::erase_if(faces, [&](const Face& face) {
                return !genders.contains(face.gender());
            });
            return faces;
        }

        std::vector<Face> filter_by_age(std::vector<Face> faces, const unsigned int& age_start, const unsigned int& age_end) {
            if (faces.empty()) {
                return faces;
            }

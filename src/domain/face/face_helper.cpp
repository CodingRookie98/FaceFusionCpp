...
std::vector<int> apply_nms(const std::vector<cv::Rect2f>& boxes, std::vector<float> confidences,
    ...
    for (size_t i = 0; i < num_box; ++i) {
        ...
        if (is_suppressed[idx_i]) { continue; }
        for (size_t j = i + 1; j < num_box; ++j) {
            size_t idx_j = indices[j];
            if (is_suppressed[idx_j]) { continue; }
    ...
    for (size_t i = 0; i < num_box; ++i) {
        ...
        if (!is_suppressed[idx]) { keep_inds.emplace_back(static_cast<int>(idx)); }
    }
    return keep_inds;
}
...
std::tuple<cv::Mat, cv::Mat> warp_face_by_face_landmarks_5(
    ...
    cv::Mat affine_matrix =
        estimate_matrix_by_face_landmark_5(face_landmark_5, warp_template, crop_size);
    cv::Mat crop_vision;
    cv::warpAffine(temp_vision_frame, crop_vision, affine_matrix, crop_size, cv::INTER_AREA,
                   cv::BORDER_REPLICATE);
...
std::tuple<cv::Mat, cv::Mat> warp_face_by_face_landmarks_5(
    ...
    return warp_face_by_face_landmarks_5(temp_vision_frame, face_landmark_5, warp_template,
                                         crop_size);
}
...
std::tuple<cv::Mat, cv::Mat> warp_face_by_translation(const cv::Mat& temp_vision_frame,
                                                      ...
                                                      const cv::Size& crop_size) {
    cv::Mat affine_matrix =
        (cv::Mat_<float>(2, 3) << scale, 0.f, translation[0], 0.f, scale, translation[1]);
    cv::Mat crop_img;
...
cv::Mat paste_back(const cv::Mat& temp_vision_frame, const cv::Mat& crop_vision_frame,
    ...
    warpAffine(crop_vision_frame, inverse_vision_frame, inverse_matrix, temp_size, cv::INTER_LINEAR,
               cv::BORDER_REPLICATE);
    ...
    channel_mats[0] = inverse_mask.mul(inverse_vision_frame_bgrs[0])
                    + temp_vision_frame_bgrs[0].mul(1 - inverse_mask);
    channel_mats[1] = inverse_mask.mul(inverse_vision_frame_bgrs[1])
                    + temp_vision_frame_bgrs[1].mul(1 - inverse_mask);
    channel_mats[2] = inverse_mask.mul(inverse_vision_frame_bgrs[2])
                    + temp_vision_frame_bgrs[2].mul(1 - inverse_mask);
...
std::vector<std::array<int, 2>> create_static_anchors(const int& feature_stride,
    ...
    for (int i = 0; i < stride_height; ++i) {
        for (int j = 0; j < stride_width; ++j) {
            ...
            for (int k = 0; k < anchor_total; ++k) { anchors.push_back({y, x}); }
        }
    }
    return anchors;
}
...
std::vector<float> calc_average_embedding(const std::vector<std::vector<float>>& embeddings) {
    ...
    for (const auto& embedding : embeddings) {
        for (size_t i = 0; i < embedding.size(); ++i) { average_embedding[i] += embedding[i]; }
    }
    for (float& value : average_embedding) { value /= static_cast<float>(embeddings.size()); }
...
std::tuple<cv::Mat, cv::Size> create_rotated_mat_and_size(const double& angle,
    ...
    cv::Mat rotated_mat =
        cv::getRotationMatrix2D(cv::Point2f(static_cast<float>(src_size.width) / 2.f,
                                            static_cast<float>(src_size.height) / 2.f),
                                angle, 1.0);
    const cv::Rect2f bbox =
        cv::RotatedRect(cv::Point2f(), src_size, static_cast<float>(angle)).boundingRect2f();
...
cv::Rect2f transform_bbox(const cv::Rect2f& bbox, const cv::Mat& affine_matrix) {
    const std::vector<cv::Point2f> points = {{bbox.x, bbox.y},

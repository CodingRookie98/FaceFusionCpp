...
6. **动态任务队列管理 (Task Queue Workbench)**：管线提交直接入队（“添加到任务队列”），左侧任务队列支持动态上移/下移排队优先级及一键取消任务。
...
| 交互范式 | 3 栏沉浸式工作台 + 底部浮动 HUD | 视线流：左(素材/队列) ➔ 中(全态画布/对比) ➔ 右(管线/入队) ➔ 底(遥测/进度) |
...
| **F15**| **添加到任务队列与队列面板** | 右侧 CTA 改为“添加到任务队列”，左侧栏聚合任务队列面板与任务计数徽标 | **已落地 (M5)** |

...
| :--- | :--- | :--- | :--- | :--- |
| **V0.7.0** | 2026-08-27 | AI Agent | 王辉 | Web 功能设计评估（grill-me 对齐）：确认维持单进程 C++ 内嵌架构（否决 Go 后端 + IPC 方案）；采纳改进项——任务 JSON 快照持久化、任务超时（默认 1h 可配置）、队列上限（默认 100 可配置）、Bearer Token 鉴权、任务命名/备注、OpenAPI 文档、前端 ErrorBoundary + Toast、旧版 Pages 清理；记录未来规划清单（ZIP 下载、i18n、移动端适配等 11 项）。 |
...
## 8. Web 功能设计评估（2026-08-27 对齐）

> 通过 grill-me 技能与用户逐层对齐（部署场景 → 优先级 → 技术方案），形成以下设计决策。实施状态见各小节标注。

### 8.1 架构决策：维持单进程 C++ 内嵌（ADR）

**决策**：维持现有单进程 C++ 内嵌 Drogon 架构，**不采用** Go 后端 + C++ 核心 + 进程间通信（IPC）方案。

**评估结论**（Go 方案被否决的理由）：
1. **破坏"单二进制、零配置"核心卖点**：两进程引入进程编排/启动顺序/崩溃重启/僵尸清理，部署体验退化；
2. **图像帧跨进程传输是性能杀手**：4K 帧 ~33MB，当前内嵌模式零拷贝直达推理；拆进程后每帧需 IPC 序列化，长视频任务累计 GB 级拷贝；
3. **推倒已投入的 Web 层**：`app.web`（web_server/task_manager/WS）+ 全部集成测试已落地验证，重写成本高；
4. **Go 高并发优势与场景错位**：推理是串行 GPU 任务，瓶颈在显存/算力而非 Web 并发，14 个 API 对 Drogon 绰绰有余。

**适用条件**（出现任一才考虑拆进程）：SaaS 化/多租户（数千并发连接）、Web 团队与核心团队分离、C++ 核心被多服务复用。
...
| # | 改进项 | 方案要点 | 优先级 | 状态 |
|---|---|---|---|---|
| 1 | **任务持久化** | JSON 文件快照（每任务一个文件）；重启恢复语义：queued→恢复排队、running→标记 failed（如实反映，避免假续跑）、终态保留 | A 健壮性 | **实施中** |
| 2 | **任务超时机制** | `web.max_execution_seconds`（默认 3600=1h）可配置；超时标记 failed、保留已产出部分帧 | A 健壮性 | **实施中** |
| 3 | **队列上限** | `web.max_queue_size`（默认 100）可配置；超限拒绝策略 `429` | A 健壮性 | 待实施 |
| 4 | **任务命名/备注** | 提交时填 `name` 字段，队列卡片与历史按名称显示（局域网多人协作识别） | D 新功能 | 待实施 |
| 5 | **OpenAPI 文档** | `/api/openapi.json` 自动生成，API 自文档化 | D 新功能 | 待实施 |
| 6 | **Bearer Token 鉴权** | 启动自动生成 token（日志 + app.yaml）；前端登录页 + localStorage；全端点中间件校验 | B 安全 | 待实施 |
| 7 | **前端 ErrorBoundary + Toast** | 杜绝组件报错整页白屏；Toast 通知系统替换 `window.alert` | C 体验 | 待实施 |
| 8 | **旧版 Pages 清理** | 删除 `TaskCreatePage/TaskListPage/TaskDetailPage` 死代码 | C 体验 | 待实施 |

### 8.3 未来规划（不纳入当前轮次，按需评估）

| # | 规划项 | 说明 | 触发条件 |
|---|---|---|---|
| P1 | 结果 ZIP 批量下载 | 任务完成后打包全部结果下载 | 批量使用场景出现 |
| P2 | 上传视觉进度条 | AssetPool 批量上传 `{cur}/{total}` 数字升级为视觉进度条 | 大文件上传体验优化 |
| P3 | 历史任务分页 | HistoryModal 一次性渲染全部任务，量大时性能隐患 | 任务量增长 |
| P4 | 人脸置信度过滤 UI | 画布提供置信度阈值调节 | 检测精度需求 |
| P5 | i18n 国际化 | 当前 UI 硬编码中文，需架构改造 | 多语言用户出现 |
| P6 | 移动端/响应式适配 | 三栏固定宽度布局重构 | 移动端使用需求 |
| P7 | 键盘快捷键 | 人脸选择/模式切换/提交任务快捷键 | 专业用户效率需求 |
| P8 | 管线编辑撤销/重做 | addStep/removeStep/moveStep undo 历史 | 编辑体验增强 |
| P9 | 批量任务提交 API | `POST /api/tasks/batch` 一次提交多个 | 无头/脚本化集成需求 |
| P10 | Webhook 回调 | 任务结束 POST 到指定 URL | 外部系统集成需求 |
| P11 | 配置热更新 | Web API 修改配置实时生效 | 运维需求出现 |

### 8.4 评估方法论（回顾）

- **场景校准**：局域网共享（默认 `0.0.0.0` 绑定 + 无鉴权，安全需求优先于单机场景）；
- **优先级排序**：后端健壮性 A > 新功能 D > 安全 B > 前端体验 C；
- **预防性评估**：用户暂无实际痛点，改进以防御性设计为主（持久化/超时/上限），避免过度设计（低优先级项全部移入 8.3 未来规划）。

<!-- AI_CONTEXT
你是一名严格的代码质量审计师 (Code Quality Auditor)。
你的目标是根据客观证据对 C++ 模块进行"无情"的评估。
原则：没有证据 = 0 分。
⚠️ TDD 审计：必须验证代码是否通过 TDD 流程产生（测试先于实现）。
-->

# C++ 代码质量评估报告

> **文档控制信息 (Document Control)**
> - **文档标识 (Document ID)**: FFC-DEV-TPL-EVAL-2026
> - **当前版本 (Version)**: V1.0.0
...
> **标准参考**:
> *   评估标准与工具指南请参考: [C++代码质量与评估标准指南](../zh/process/C++_quality_standard.md)
> *   TDD 与开发规范: [AGENTS.md](../../../AGENTS.md)

## 0. 评估前准备 (Pre-Evaluation)
> **指令**: 在开始评估前，必须完成以下动作。

- [ ] 我已通读 `{模块名称}` 的所有源码 (.ixx, .cpp)。
- [ ] 我已运行 Clang-Tidy 并获取了报告 (Windows + MSVC 环境跳过)。
- [ ] 我已运行单元测试并获取了覆盖率数据。
- [ ] **TDD 审计**: 我已检查 git 历史，确认测试提交先于/同时于实现提交。
...
| **代码路径** | `{代码路径}` |
| **C++标准** | C++20 |
| **构建工具** | CMake + MSVC/GCC/Clang + Ninja |
| **负责人** | `{负责人}` |
...
| 检查项 | 规范要求 | 状态 | 说明/扣分理由 (非满分必填) | 证据来源 |
| :--- | :--- | :--- | :--- | :--- |
...
### 4.1 TDD 实践评估 (权重 5%)
> **核心原则**: 验证代码是否通过 TDD 流程产生，而非事后补测试。

| 检查项 | 评分点 | 状态 | 说明/扣分理由 (非满分必填) | 证据来源 |
| :--- | :--- | :--- | :--- | :--- |
| **测试先行** | 测试提交时间 ≤ 实现提交时间 | ✅ / ⚠️ / ❌ | 检查 git log 时间戳，测试文件是否先于/同时于实现文件 | `git log --oneline --date-order -- tests/ src/` |
| **Red-Green 证据** | 存在失败→通过的测试历史 | ✅ / ⚠️ / ❌ | CI/CD 历史是否显示 红→绿 转变 | CI 构建日志或本地 git bisect 验证 |
| **行为测试** | 测试 "做什么" 而非 "怎么做" | ✅ / ⚠️ / ❌ | 测试是否依赖实现细节（如私有方法、内部状态） | 代码审查：测试是否仅通过 public API 验证 |
| **测试独立性** | 每个测试可独立运行 | ✅ / ⚠️ / ❌ | 测试间是否有顺序依赖或共享状态 | 随机顺序执行测试：`--gtest_shuffle` |
| **重构安全** | 重构后测试仍通过 | ✅ / ⚠️ / ❌ | 代码重构是否破坏了测试（说明测试质量差） | 检查重构相关 commit 的测试状态 |

**TDD 违规记录** (如有):
| # | 违规类型 | 文件/函数 | 说明 |
|:--|:---------|:----------|:-----|
| 1 | {先写实现后补测试 / 无测试 / 事后修改测试} | `{file}` | {具体情况} |
...
### 5.1 评分汇总表
> 评分计算请参考 [C++质量标准指南](../zh/process/C++_quality_standard.md#2-评分标准)

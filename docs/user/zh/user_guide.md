...
> - **文档标识 (Document ID)**: FFC-USER-ZH-UG-2026
> - **当前版本 (Version)**: V1.0.0
...
* **关键参数 (`face_selector_mode`)**:
  ...
  * `reference`: 高级玩法。需配套设置 `reference_face_path`。程序只处理与参考照片相似的人脸。
    * **透传机制 (Pass-through)**: 若源图中找不到人脸，或画面中没有匹配的人脸（报错码 `E403`），程序将**保留原帧并直接输出**。这确保了视频渲染不中断且音画绝对同步。
...
* **分块处理 (Tile Processing)**: 处理 4K/8K 等高分辨率画面时，为了防止显存溢出 (OOM)，系统会自动将画面切分为固定大小的 **Tile (图块)** 进行接力处理。
    * **优势**: 即使是 8GB 显存，也能通过分块策略处理 4K 分辨率的超分辨率任务。
...
## 4. 性能与资源策略 (Performance & Resource Strategies)

FaceFusionCpp 允许您根据硬件配置调整运行策略，以平衡速度与显存占用。

### 4.1 内存管理策略 (`memory_strategy`)

*   **Strict (严格模式)**:
    *   **行为**: 仅在需要时加载模型，处理完立即尝试释放。
    *   **优点**: 显存占用极低。适合低显存显卡（如 4GB/6GB）。
    *   **缺点**: 频繁加载模型会导致处理速度下降。
*   **Tolerant (宽容模式)**:
    *   **行为**: 模型加载后常驻显存。
    *   **优点**: 处理速度最快，无重复加载开销。
    *   **缺点**: 持续占用显存。适合显存充足的环境（如 8GB+）。

### 4.2 执行顺序策略 (`execution_order`)

*   **Sequential (顺序模式 - 默认)**:
    *   每一帧依次跑完流水线所有步骤。
    *   **适用**: 侧重低延迟，且显存足以同时放下所有启用的模型。
*   **Batch (批处理模式)**:
    *   所有帧先跑完 A 处理器，再统一跑 B 处理器。
    *   **优势**: 配合 `strict` 内存模式，可实现"单模型显存占用"，在大显存模型堆叠时能极大降低硬件门槛。
    *   **注意**: 处理长视频时需要较大的磁盘或内存空间存储中间结果。
